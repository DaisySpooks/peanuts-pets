...
export default function AquariumBackground() {
  return (
    <div aria-hidden="true" className="pointer-events-none absolute inset-0">
      ...
      <span className="absolute bottom-0 left-[82%] h-1 w-1 rounded-full bg-cream/40 animate-bubble-rise motion-ambient [animation-delay:0.8s]" />
      <span className="absolute bottom-0 left-[45%] hidden h-1 w-1 rounded-full bg-cream/30 animate-bubble-rise motion-ambient [animation-delay:3.4s] md:inline-block" />
      <span className="absolute bottom-0 left-[92%] hidden h-1.5 w-1.5 rounded-full bg-cream/30 animate-bubble-rise motion-ambient [animation-delay:1.9s] md:inline-block" />
      <span className="absolute left-[20%] top-[22%] h-1 w-1 rounded-full bg-gold/40 animate-mote-drift motion-ambient" />
      <span className="absolute right-[18%] top-[35%] h-1 w-1 rounded-full bg-gold/30 animate-mote-drift motion-ambient [animation-delay:2s]" />
      <span className="absolute left-[8%] top-[60%] hidden h-1 w-1 rounded-full bg-gold/25 animate-mote-drift motion-ambient [animation-delay:4s] md:inline-block" />
      <span className="absolute right-[10%] top-[68%] hidden h-1 w-1 rounded-full bg-cream/20 animate-mote-drift motion-ambient [animation-delay:1s] md:inline-block" />
    </div>
  )
}
